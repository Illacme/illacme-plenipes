...
async def test_dry_run_backend_engine():
    ...
    assert any("未提供授权密钥" in log["message"] for log in res_no_key["logs"])
    ...
    assert any("物理出版演练圆满完成" in log["message"] for log in res_success["logs"])

    # 4. 测试 WordPress 专属字段成功场景 (api_url, application_password)
    payload_wp = {
        "id": "wordpress",
        "settings": {
            "enabled": True,
            "api_url": "https://yoursite.com/wp-json/wp/v2",
            "application_password": "wp_app_pwd_here"
        }
    }
    res_wp = await dry_run_plugin(payload_wp)
    assert res_wp["success"] is True
    assert any("物理出版演练圆满完成" in log["message"] for log in res_wp["logs"])
    ...
    assert any("物理出版演练圆满完成" in log["message"] for log in res_medium["logs"])
