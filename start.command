#!/bin/bash
# ==============================================================================
# 🚀 Illacme Plenipes - macOS 一键双击启动器 (One-Click Launcher)
# 职责：自动检测 Python 3.10+ 与虚拟环境，拉起出版引擎并自动唤醒治理中心看板。
# ==============================================================================
...
if [ -z "$PY_CMD" ]; then
    ...
    echo -e "${YELLOW}💡 请先安装 Python 3.10 或更高版本 (推荐使用 https://www.python.org 或 brew install python@3.12)${RESET}"
    read -p "按回车键退出..."
    exit 1
fi
...
if [ -n "$MISSING_DEP" ]; then
    ...
    if [ $? -ne 0 ]; then
        echo -e "${RED}❌ 依赖安装失败，请手动检查网络连接或执行: pip install -r requirements.txt${RESET}"
        read -p "按回车键退出..."
        exit 1
    fi
...
# 5. 异步唤醒默认浏览器打开治理中心 (延时 2.5 秒等待 API 网关启动)
(
    sleep 2.5
    if command -v open &>/dev/null; then
        open "http://localhost:43212"
    fi
) &

echo -e "${CYAN}🛰️ 正在点火拉起 Illacme Plenipes 全球出版引擎...${RESET}"
echo -e "${YELLOW}💡 治理中心即将自动在浏览器中弹出 (http://localhost:43212)${RESET}"
