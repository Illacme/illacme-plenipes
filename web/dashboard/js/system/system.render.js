        ...
        return `
            <div class="category-header-banner" style="display: flex; flex-direction: column; gap: 8px; margin-bottom: 20px; padding: 18px 22px; background: rgba(0, 242, 255, 0.03); border: 1px solid var(--glass-border); border-radius: 12px; backdrop-filter: blur(10px);">
                ...
                <div id="gen-panel-storage" style="display: ${activeSub === 'storage' ? 'block' : 'none'};">
                    ...
                    ${typeof window.renderGranularCacheHub === 'function' ? window.renderGranularCacheHub() : ''}
                ...
                <div id="gen-panel-engine" style="display: ${activeSub === 'engine' ? 'block' : 'none'};">
                    <div class="settings-group">
                        <div class="settings-grid">
                            ${renderSettingsItem('首页出版领航仪自动展开', 'ui.launchpad_auto_open', (typeof window.shouldAutoOpenLaunchpad === 'function' ? window.shouldAutoOpenLaunchpad() : true), 'checkbox', {
                                onchange: `window.setLaunchpadAutoOpenPreference(this.checked); if(typeof window.addAudit==='function') window.addAudit('🚀 首页出版领航仪自启偏好已更新为: ' + (this.checked ? '开启' : '关闭'), 'info');`,
                                description: '控制进入工作台首页（概览）时是否自动展开全屏出版领航仪（Publishing Navigator）。关闭后进入首页将直接展示 3D 知识星系宇宙，随时可按键盘快捷键 I 或点击顶部 Logo 唤起。',
                                effectiveness: 'live'
                            }, 'local')}
                            ${renderSettingsItem('知识星谱默认自动自转', 'ui.galaxy_auto_rotate', (typeof window.shouldAutoRotateGalaxy === 'function' ? window.shouldAutoRotateGalaxy() : false), 'checkbox', {
                                onchange: `window.setGalaxyAutoRotatePreference(this.checked); if(typeof window.addAudit==='function') window.addAudit('🪐 知识星谱默认自转偏好已更新为: ' + (this.checked ? '开启' : '关闭'), 'info');`,
                                description: '控制进入工作台首页（知识星谱）时，3D 知识关联星图是否默认开启平滑壮观的环绕轨道自转。关闭后进入星谱将保持静止观赏状态；您亦可在星图左上角指标卡片或底部操作栏一键切换自转。',
                                effectiveness: 'live'
                            }, 'local')}
