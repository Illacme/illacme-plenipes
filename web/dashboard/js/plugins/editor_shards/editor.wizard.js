    ...
    window.getPluginWizardSteps = (pluginId, category = '') => {
        ...
        if (cat === 'image_hosting') {
            const imageHostingMap = {
                ...
                'cloudflare_r2': ['1. Account ID 与 API 令牌', '2. 存储空间 (Bucket)', '3. 自定义加速域名 (可选)', '4. 上传测试与保存'],
                'imgbb': ['1. API 访问密钥 (Key)', '2. 代理参数 (可选)', '3. 上传测试与保存'],
                'catbox': ['1. 个人 Userhash (可选)', '2. 代理参数 (可选)', '3. 上传测试与保存'],
                'telegraph': ['1. 开源部署向导', '2. 自建节点端点 (Endpoint)', '3. 独立代理参数 (可选)', '4. 上传测试与保存'],
        ...
        const specificStepsMap = {
            // === 全站托管 (Hosting) ===
            'github_pages': ['1. 访问令牌 (只需填 Token)', '2. 目标仓库与分支 (选填)', '3. 高级参数 (可选)', '4. 连通测试与保存'],
            ...
            // === 网络穿透 (Tunnel) ===
            'cloudflare': ['1. 隧道令牌 (可选留空)', '2. 专属域名与网络策略 (选填)', '3. 连通测试与保存'],
