...
/**
 * 💡 [创作者体验优化] 智能推断配置项的人性化白话说明，彻底替代生硬晦涩的技术黑话
 */
window.resolveSmartDescription = (label, path) => {
    ...
    if (p.includes('prefix') || l.includes('prefix') || l.includes('路径前缀') || l.includes('前缀')) {
        return '文件上传至存储桶时的可选相对子目录路径前缀（留空则部署于根目录）。';
    }
    if (p.includes('acl') || l.includes('acl') || l.includes('访问控制')) {
        return '上传对象在云存储中的访问控制权限（通常设为 public-read）。';
    }

    // 5. 主机 / 端口 / 目录 / 用户
    if (p.includes('host') || l.includes('host') || l.includes('主机') || l.includes('ip')) {
        return '远程服务器或主机的连接域名或 IP 地址。';
    }
    if (p.includes('port') || l.includes('port') || l.includes('端口')) {
        return '远程服务的网络连接端口（留空通常使用协议默认端口）。';
    }
    if (p.includes('remote_dir') || l.includes('remote dir') || l.includes('远程目录') || l.includes('部署目录')) {
        return '静态网页文件上传部署至远程服务器的绝对目标路径（例如 /var/www/html）。';
    }
    if (p.includes('username') || p.includes('user') || l.includes('username') || l.includes('用户名') || l.includes('账号')) {
        return '用于登录远程主机或执行发布操作的账户名。';
    }
