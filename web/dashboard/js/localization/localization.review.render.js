...
function _renderParaBlock(p) {
    ...
    const editedBadge = p._edited ? '<span class="edited-mark" style="color:#ffb300; font-size:0.7rem; font-weight:600; padding:1px 6px; background:rgba(255,179,0,0.12); border-radius:4px; border:1px solid rgba(255,179,0,0.25);">✏️ 已修改</span>' : '';
    ...
    return `<div class="review-para-header">${editedBadge}${retransBtn}</div><div class="review-para-text">${_escapeHtml(p.text)}</div>`;
}
