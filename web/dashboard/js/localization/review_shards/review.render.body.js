/**
 * 🔒 [I5] Illacme Plenipes Translation Review Body Rendering Module
 * 职责：翻译校对工作台的抽屉外层、语言 Tabs、警报条及三栏（译文、预览、原文）主体结构渲染。
...
function _reviewRenderBody() {
    ...
    if (!isMissing) {
        ...
        previewHtml = `<div style="padding:20px; display:flex; flex-direction:column; gap:16px;"><div class="review-field" style="margin:0;"><label>👁️ 译文预览 (Preview)</label></div><div class="preview-markdown-content" style="color:var(--text-bright);"><div class="preview-title" style="margin-bottom:20px;">${renderPreviewTitle}</div><div id="preview-paras-container">${targetParas.map((p, idx) => `<div id="preview-para-${p.index >= 0 ? p.index : 'spacer-' + idx}" class="preview-para-item">${markdownParser((typeof _reviewRewriteMarkdown === 'function') ? _reviewRewriteMarkdown(p.text || '', state.docId, p.index >= 0 ? (sourceParaByIndex[p.index] || '') : '') : (p.text || ''))}</div>`).join('')}</div></div></div>`;
    }
