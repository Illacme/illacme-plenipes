...
function formatLessonRow(l) {
    let displayTime = '最近例行校验', cat = 'SOVEREIGNTY_SHIELD', rawTitle = '', filePath = '全站出版流程';
    if (typeof l === 'string') {
        rawTitle = l;
    ...
    const cleanTitle = rawTitle.replace(/主权标签/g, '专属标记').replace(/主权屏护/g, '品牌标记').replace(/主权/g, '专属');
    const labelText = LESSON_LABELS[cat] || cat;
    const styleConfig = LESSON_COLORS[cat] || { bg: "rgba(0, 242, 255, 0.1)", border: "var(--glass-border)", text: "var(--accent-secondary)" };

    return `
        <tr style="border-bottom: 1px solid var(--glass-border); transition: background 0.2s ease;">
            <td style="padding: 10px 12px; color: var(--text-dim); white-space: nowrap; font-size: 0.74rem;">${displayTime}</td>
            <td style="padding: 10px 12px; white-space: nowrap;">
                <span style="background: ${styleConfig.bg}; border: 1px solid ${styleConfig.border}; color: ${styleConfig.text}; padding: 3px 8px; border-radius: 4px; font-size: 0.7rem; font-weight: 600; display: inline-block;">${labelText}</span>
            </td>
...
window.renderAiLessonsVisualizer = (summary, lessons, container) => {
    ...
    let maxCat = '暂无记录', maxVal = 0;
    for (const [cat, count] of Object.entries(catCounts)) {
        if (count > maxVal) {
            maxVal = count;
            maxCat = LESSON_LABELS[cat] || cat;
        }
    }
    if (maxVal > 0) maxCat = `${maxCat} (${maxVal} 次)`;
    ...
    if (total === 0 && (!lessons || lessons.length === 0)) {
        progressBarsHtml = `
            <div style="color: var(--text-dim); text-align: center; padding: 25px 15px; font-size: 0.82rem; background: var(--bg-glass, rgba(255,255,255,0.01)); border-radius: 8px; border: 1px dashed var(--glass-border); display: flex; flex-direction: column; align-items: center; gap: 10px;">
                ...
                <button type="button" class="mini-btn glow-btn" onclick="window.seedAiLessonsHistory()" style="padding: 6px 14px; background: var(--accent-primary); color: #ffffff !important; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 0.78rem;">🧪 模拟生成测试修复日志</button>
            </div>`;
    } else {
        progressBarsHtml = Object.entries(catCounts).map(([cat, count]) => {
            const pct = total > 0 ? ((count / total) * 100).toFixed(1) : 0;
            const label = LESSON_LABELS[cat] || cat;
            const styleConfig = LESSON_COLORS[cat] || { bg: "rgba(0, 242, 255, 0.1)", border: "var(--glass-border)", text: "var(--accent-secondary)" };
            return `
    ...
    container.innerHTML = `
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 16px; align-items: stretch; margin-bottom: 20px;">
            <div style="background: var(--bg-glass, rgba(255,255,255,0.02)); border: 1px solid var(--glass-border); border-radius: 10px; padding: 18px; flex: 1; min-width: 280px; display: flex; flex-direction: column;">
                <h4 style="margin: 0 0 14px 0; font-size: 0.85rem; color: var(--text-bright, #ffffff); border-bottom: 1px solid var(--glass-border); padding-bottom: 10px; font-weight: 700; display: flex; align-items: center; gap: 6px;">📊 修复类型分布比例</h4>
                <div style="flex: 1; display: flex; flex-direction: column; justify-content: center;">${progressBarsHtml}</div>
            </div>
            <div style="background: var(--bg-glass, rgba(255,255,255,0.02)); border: 1px solid var(--glass-border); border-radius: 10px; padding: 18px; flex: 2; min-width: 440px; display: flex; flex-direction: column;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; border-bottom: 1px solid var(--glass-border); padding-bottom: 10px;">
                    <h4 style="margin: 0; font-size: 0.85rem; color: var(--text-bright, #ffffff); font-weight: 700; display: flex; align-items: center; gap: 6px;">📜 AI 自动修复日志账本</h4>
                ...
                <div style="display: flex; gap: 10px; margin-bottom: 14px; align-items: center; flex-wrap: nowrap;">
                    ...
                    <select id="lesson-cat-filter" style="flex: 0 0 190px; height: 36px; box-sizing: border-box; background: var(--bg-solid, rgba(0,0,0,0.2)); border: 1px solid var(--glass-border); color: var(--text-bright, #ffffff); padding: 0 10px; border-radius: 6px; font-size: 0.78rem; cursor: pointer; outline: none;">
                        ...
                        ${Object.entries(LESSON_LABELS).map(([cat, text]) => `<option value="${cat}">${text}</option>`).join('')}
                    </select>
                </div>
    ...
    const updateFilter = () => {
        ...
        filteredLessons = sourceList.filter(l => {
            if (!l) return false;
            let errorTitle = '', cat = 'SOVEREIGNTY_SHIELD', contextStr = '';
            if (typeof l === 'string') {
                errorTitle = l;
            ...
            const queryMatch = !query || errorTitle.toLowerCase().includes(query) || cat.toLowerCase().includes(query) || contextStr.toLowerCase().includes(query);
        ...
        if (!filteredLessons || filteredLessons.length === 0) {
            ...
            if (statusText) statusText.innerText = `当前筛选共 0 条 / 历史总沉淀 ${sourceList.length} 条`;
            return;
        }

        tbody.innerHTML = filteredLessons.map(formatLessonRow).join('');
        if (statusText) statusText.innerText = `当前筛选共 ${filteredLessons.length} 条 / 历史总沉淀 ${sourceList.length} 条`;
    };
