...
# -*- coding: utf-8 -*-
"""
Illacme Plenipes — Render Publisher Plugin
🚀 [V48.5]：通过 Git 增量推送与 Trigger Deploy Hook (Webhook POST) 独立触发 Render 云端构建与静态站点发布。
"""
...
class RenderPublisher(BasePublisher, GitPushMixin):
    """
    🚀 [V48.5] Render 发布插件
    """
    PLUGIN_ID = "render"
    DISPLAY_NAME = "Render"
    VERSION = "V2.0"
    DESCRIPTION = "独立部署静态站点至 Render 并触发云端构建。"
    ...
    def push(self, bundle_path: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
        """
        🚀 执行物理发布：先执行自包含 Git 推送（如配置），再向 Render 的 Deploy Hook URL 发送 HTTP POST 请求。
        """
        if not self.deploy_hook_url:
            return {"status": "skipped", "message": "Render deploy_hook_url not configured."}
        ...
        copied_count = 0
        if self.repo_url:
            tlog.info("🚀 [Render] 检测到关联 Git 仓库，正在执行 Git 增量差分推送...")
            import tempfile
            work_dir = tempfile.mkdtemp(prefix="plenipes_render_")
            try:
                # 克隆或初始化分支
                ...
                commit_msg = f"deploy(render): {timestamp} (files: {copied_count})"
                pushed = self._commit_and_push(
                    work_dir, commit_msg, self.branch, self.repo_url, self.token,
                    force_push=self.force_push
                )
                if pushed:
                    tlog.success(f"🟢 [Render] Git 增量推送成功！已推送 {copied_count} 个文件至 {self.branch} 分支。")
                else:
                    tlog.info("ℹ️ [Render] Git 无变更需要推送。")
                ...
                tlog.error(f"❌ [Render] Git 操作失败: {masked_err}")
                return {"status": "error", "message": f"Git operation failed: {suggested_err}"}
            except Exception as e:
                err_msg = str(e)
                masked_err = self._mask_url_credentials(err_msg, self.token)
                suggested_err = self._add_autotherapy_suggestion(masked_err)
                tlog.error(f"❌ [Render] 部署异常: {masked_err}")
        ...
        tlog.info("🚀 [Render] 正在向对端 Deploy Hook 发送部署信号...")
        try:
            proxy = self.get_proxy()
            proxies = {"http": proxy, "https": proxy} if proxy else None
            resp = requests.post(self.deploy_hook_url, proxies=proxies, timeout=10)
            resp.raise_for_status()
            tlog.success("✅ [Render] 部署信号发送成功！已触发云端流水线编译。")
            return {
                "status": "success",
                "message": f"Render deploy hook triggered. HTTP {resp.status_code}",
                "url": self.get_deploy_url(),
                "files": copied_count
            }
        except Exception as e:
            tlog.error(f"❌ [Render] 触发部署失败: {e}")
