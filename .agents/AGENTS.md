...
*   **适用场景**：所有涉及 `route_matrix` 频道路由矩阵、多语言导航定制、官方槽位标准字典 (`COMMON_SLOT_I18N` / `SLOT_I18N_FALLBACK`) 及 AI 导航翻译的代码与配置修改。
*   **物理红线**：
    1. **官方模板槽位字典绝对契约**：官方网页模板自带的 `showcase` 槽位，其在前端与后端的官方英文标准名必须物理锁定为 `Show`，严禁未经母本架构师批准擅自修改为 `Showcase` 或其它截断值。
    2. **文库与网页路径根据品牌子目录分配确定，严禁 slot 劫持业务标签**：在频道映射中，`source`（文库物理子目录）与 `prefix`（网页 URL 路径）由品牌的子目录分配策略决定，而 `nav_label`（导航显示名）承载品牌独立业务含义；当创作者为栏目定制名称（如“核心特性”、“实战手册”）时，在点击“AI 一键填充”或执行多语言翻译时，**严禁被底层技术渲染槽位（`slot`）或标准字典强行覆盖**，系统必须判定为非字典项，100% 走大模型 AI 精准翻译。
    3. **自动化守护门禁**：所有导航多语言字典与翻译逻辑修改，必须强制通过 `pytest tests/test_nav_i18n_sovereignty.py` 自动化回归门禁。

## 16. 执行异常原始日志穿透与零模糊封装铁律 (Raw Error Transparency & Zero-Obfuscation Rules)
*   **适用场景**：所有遭遇中断、异常抛错或创作者询问“报错原因/为什么终止”的排查场景。
*   **物理红线**：
