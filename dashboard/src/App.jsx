  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 1. 抓取教训流
        try {
          const lessonRes = await fetch('/api/governance/lessons');
          if (lessonRes.ok) {
        ...
        // 2. 抓取物理审计统计
        try {
          const statsRes = await fetch('/api/governance/sync-stats');
          if (statsRes.ok) {
        ...
        // 3. 抓取健康矩阵
        try {
          const healthRes = await fetch('/api/governance/health-report');
          if (healthRes.ok) {
