...
def verify_token(request: Request, x_token: Optional[str] = Header(None, alias="X-Token")) -> None:
    """
    ...
    if api_token:
        ...
        if incoming_token != api_token:
            from core.utils.event_bus import bus
            bus.emit("SECURITY_ALERT", category="API_TOKEN_INVALID", message=f"接口认证失败：来自 {request.client.host if request.client else '未知'} 的越权访问已被物理拦截。")
