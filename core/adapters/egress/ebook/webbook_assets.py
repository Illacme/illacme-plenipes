...
[data-theme="light"] {
  --bg-main: #f8fafc; --bg-sidebar: #ffffff; --bg-card: #f1f5f9; --text-main: #1e293b; --text-dim: #64748b;
  --text-title: #0f172a; --accent: #059669; --border: #e2e8f0; --code-bg: #f8fafc;
}
[data-theme="sepia"] {
...
body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; font-size: var(--font-size); background: var(--bg-main); color: var(--text-main); line-height: 1.8; }
...
.wb-btn { background: var(--bg-card); border: 1px solid var(--border); color: var(--text-main); border-radius: 6px; padding: 4px 9px; cursor: pointer; font-size: 0.82rem; transition: all 0.2s; }
.wb-btn:hover { border-color: var(--accent); color: var(--accent); }
.wb-controls { display: flex; gap: 6px; align-items: center; }
.wb-theme-btn { background: none; border: 1px solid var(--border); border-radius: 5px; padding: 3px 7px; cursor: pointer; font-size: 0.85rem; }
.wb-theme-btn.active { border-color: var(--accent); background: rgba(16,185,129,0.15); }
.wb-layout { display: flex; margin-top: 50px; min-height: calc(100vh - 50px); }
.wb-sidebar { width: 290px; background: var(--bg-sidebar); border-right: 1px solid var(--border); position: fixed; top: 50px; bottom: 0; left: 0; display: flex; flex-direction: column; overflow: hidden; z-index: 800; transition: transform 0.3s; }
...
.wb-search-box input { width: 100%; padding: 6px 10px; background: var(--bg-card); border: 1px solid var(--border); border-radius: 6px; color: var(--text-main); font-size: 0.84rem; outline: none; }
...
.wb-toc-item { flex: 1; display: block; padding: 6px 8px; text-decoration: none; color: var(--text-main); font-size: 0.86rem; border-radius: 6px; transition: color 0.15s; }
...
.wb-toc-subitem { display: block; padding: 3px 6px; text-decoration: none; color: var(--text-dim); font-size: 0.81rem; border-radius: 4px; }
...
.wb-main { flex: 1; margin-left: 290px; padding: 30px 40px 100px; transition: margin 0.3s; }
.wb-sidebar.collapsed ~ .wb-main { margin-left: 0; }
.wb-content-wrapper { max-width: 860px; margin: 0 auto; transition: max-width 0.3s, opacity 0.18s ease-in-out; }
.wb-crossfade { opacity: 0; transform: translateY(4px); }
.wb-layout-wide .wb-content-wrapper { max-width: 1360px; }
.wb-chapter { margin-bottom: 70px; padding-bottom: 40px; border-bottom: 1px solid var(--border); scroll-margin-top: 95px; }
...
.wb-chapter-body p { margin: 0.8em 0; }
.wb-chapter-body h1, .wb-chapter-body h2, .wb-chapter-body h3, .wb-chapter-body h4 { color: var(--text-title); margin: 1.4em 0 0.6em; scroll-margin-top: 95px; }
...
.wb-chapter-body a { color: var(--accent); text-decoration: none; }
...
.wb-compare-chip.active { background: rgba(16,185,129,0.15); border-color: var(--accent); color: var(--accent); font-weight: 700; box-shadow: 0 0 8px rgba(16,185,129,0.2); }

/* 📖 单语言沉浸模式 (默认，未勾选对照栏时) */
body:not(.wb-concordance) .wb-poly-header { display: none !important; }
body:not(.wb-concordance) .wb-poly-subtitles { display: none !important; }
...
@media (max-width: 900px) {
  ...
  if (btn && sb) {
    btn.onclick = () => { if (window.innerWidth <= 900) { sb.classList.toggle('open'); } else { sb.classList.toggle('collapsed'); } };
  }
  ...
  if (inc) inc.onclick = () => { fs = Math.min(24, fs + 1); document.documentElement.style.setProperty('--font-size', fs + 'px'); };
  if (dec) dec.onclick = () => { fs = Math.max(13, fs - 1); document.documentElement.style.setProperty('--font-size', fs + 'px'); };
  ...
  const langNames = { zh: '🇨🇳 中文', en: '🇬🇧 英语', ja: '🇯🇵 日语', fr: '🇫🇷 法语', de: '🇩🇪 德语' };
    ...
    document.querySelectorAll('.wb-polyglot-block').forEach(block => {
      block.querySelectorAll('.wb-poly-item').forEach(item => {
        ...
        if (pos >= 0) {
          ...
          if (badge) {
            badge.textContent = l === primaryLang ? `${l.toUpperCase()} 主语言` : `${l.toUpperCase()} 对照栏`;
          }
        } else {
  ...
  primaryBtns.forEach(btn => {
    btn.onclick = () => {
      ...
      primaryLang = l;
      compareLangs.delete(l); // 对照栏排除当前主语言
