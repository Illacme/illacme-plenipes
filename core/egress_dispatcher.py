...
class EgressDispatcher:
    def __init__(self, paths, meta, route_manager, asset_pipeline, ssg_adapter, mdx_resolver, syndicator, broadcaster, pub_cfg, fm_order, asset_base_url, i18n_cfg, janitor=None):
        ...
        # 🚀 [Governance] 注入 Project Resonance 演化批次标识
        self.ael_iter_id = "AEL-2026.04.20.RESONANCE"
    ...
    def dispatch(self, asset_index, title, slug, masked_body, fm_dict, rel_path, lang_code, route_prefix, route_source, mapped_sub_dir, masks, is_dry_run, is_target=False, node_assets=None, node_ext_assets=None, node_outlinks=None, assets_lock=None, force_persistence_date=None):
        ...
        def unmask_fn(m):
            ...
            if is_url_only_lnk or is_url_only_img:
                ...
                if target_asset_path:
                    ...
                    try:
                        ...
                        if meta:
                            alt_text = meta.get("alt_texts", {}).get(lang_code) or meta.get("alt_text", "")
                    except Exception: pass

                    print(f"DEBUG_UNMASK: lang_code={lang_code}, h={h}, meta={meta}, alt_map={alt_map}, target_code={target_code}, lang_alt={lang_alt}"); processed_name = log_asset(self.asset_pipeline.process(target_asset_path, asset_filename, is_dry_run))
                    return f"{self.asset_base_url}{processed_name}" if not is_url_only_img else f"![{alt_text}]({self.asset_base_url}{processed_name})"
                ...
                if match:
                    ...
                    if target_asset_path:
                        ...
                        try:
                            ...
                            meta = self.meta.get_asset_metadata(h)
                            if meta:
                                # 🚀 [Resonance 架构对齐]：使用标准化的语种代码进行检索
                                alt_map = meta.get("alt_texts", {})
                                target_code = str(lang_code).strip().lower()
                                
                                # 优先级链条：精确语种匹配 > 账本主 Alt > 原始匹配 Alt
                                lang_alt = alt_map.get(target_code) or alt_map.get(lang_code)
                                final_alt = lang_alt or meta.get("alt_text") or orig_alt
                        except Exception: pass
            ...
            if orig.startswith('![['):
                ...
                if target_asset_path:
                    # 🚀 [Resonance] 提取本地化 Alt
                    alt_text = orig[3:-2].split('|')[1] if '|' in orig[3:-2] else ""
                    try:
                        with open(target_asset_path, 'rb') as f:
                            h = hashlib.md5(f.read()).hexdigest()
                        meta = self.meta.get_asset_metadata(h)
                        if meta:
                            alt_text = meta.get("alt_texts", {}).get(lang_code) or alt_text or meta.get("alt_text", "")
                    except Exception: pass

                    print(f"DEBUG_UNMASK: lang_code={lang_code}, h={h}, meta={meta}, alt_map={alt_map}, target_code={target_code}, lang_alt={lang_alt}"); processed_name = log_asset(self.asset_pipeline.process(target_asset_path, filename, is_dry_run))
                ...
                if target_asset_path:
                    print(f"DEBUG_UNMASK: lang_code={lang_code}, h={h}, meta={meta}, alt_map={alt_map}, target_code={target_code}, lang_alt={lang_alt}"); processed_name = log_asset(self.asset_pipeline.process(target_asset_path, target_text, is_dry_run))
        ...
        if not is_dry_run:
            try:
                ...
                # 🚀 [Syndication Pilot]：执行全域广播与内容推送
                if lang_code == self.i18n.source.lang_code:
                    # 1. 外部平台内容分发 (LinkedIn, WordPress 等)
                    self.syndicator.syndicate(ordered_fm, final_body, f"/{lang_code}/{mapped_sub_dir}/{slug}".replace('//', '/'), ael_iter_id=self.ael_iter_id)
                    # 2. 全域 Webhook 状态广播 (飞书、钉钉、企微等)
                    self.broadcaster.broadcast(title, rel_path, lang_code, mapped_sub_dir, slug, ael_iter_id=self.ael_iter_id)
            except Exception as e:
                logger.error(f"🛑 物理或分发链路异常: {e}")
