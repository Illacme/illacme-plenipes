...
### 3.6 逻辑主权与契约化适配器 (Logic Sovereignty & Contractual Adapters)
为了确保核心业务逻辑（如 Prompt 组装、数据清洗、SEO 提取）不随适配器种类的增加而碎片化，系统采用了“逻辑主权隔离”模式：
- **BaseTranslator (大脑层)**：掌握 100% 的业务主权，负责处理所有通用的 AI 工作流。
- **Provider Adapters (协议层)**：仅作为纯净的协议实现者，通过原子接口 `_ask_ai` 执行具体的模型请求（如 OpenAI/Gemini/DeepSeek）。
- **治理校验**：通过 AST 审计强制拦截在适配器中篡改业务逻辑的行为，确保引擎的工业级稳定性。

## 4. 扩展性原则

*   **新增语种**：在 `i18n_settings` 中配置 `targets` 阵列，无需修改核心代码。
*   **新增 AI 模型适配**：在 `core/adapters/ai/` 下新建适配器类，仅需实现 `_ask_ai` 原子协议接口。
*   **新增 SSG 框架**：在 `core/adapters/egress/ssg/` 中定义新的渲染映射。
