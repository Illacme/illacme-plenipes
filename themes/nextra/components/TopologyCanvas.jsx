/**
 * TopologyCanvas — Nextra sidebar & fullscreen immersive graph component
...
const I18N = {
  ...
  ko: {
    ...
    btn: '전체 화면',
    hint: '💡 팁: 스크롤로 확대축소 / 드래그로 이동 / 클릭하여 문서 이동 / ESC 키 또는 오른쪽 위로 안전 종료',
    close: '✕ 전체 화면 종료 [ESC]'
  },
  zh: {
    title: '🌌 局部关系图谱',
...
const resolveNextraUrl = (u) => !u ? '/' : u.replace(/\/engineering\/engineering\//g, '/engineering/').replace(/^(\/(?:[a-zA-Z]{2,3}(?:-[A-Za-z0-9]+)?\/)?)docs\//, '$1') || '/';

export default function TopologyCanvas({ height = 260 }) {
    ...
    (async () => {
      try {
        ...
        graphRef.current = renderTopologyGraph(containerRef.current, data, {
          ...
          height,
          darkMode: isDark,
          activeUrl: router.asPath || router.pathname,
          isLocal: true,
          localDepth: 1,
          onNavigate: (url) => {
            const targetUrl = resolveNextraUrl(url);
            if (router?.push) router.push(targetUrl);
            else window.location.href = targetUrl;
          }
        });
        setLoaded(true);
      } catch (e) {
        console.warn('[Nextra:TopologyCanvas] Failed to load graph:', e);
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn('[Nextra:TopologyCanvas] Fullscreen graph failed:', e);
      }
    })();
  ...
  }, [isModalOpen, graphData, isDark, router.asPath]);
  ...
  const modalContent = isModalOpen && typeof document !== 'undefined' ? (
    ...
    >
      <div style={{
        ...
        <div>
          <div style={{ fontSize: '1.2rem', fontWeight: 700, color: isDark ? '#f8fafc' : '#0f172a', letterSpacing: '-0.01em' }}>
            {t.modalTitle}
          </div>
          <div style={{ fontSize: '0.78rem', color: isDark ? '#38bdf8' : '#0284c7', marginTop: '4px', fontWeight: 500 }}>
            {t.hint}
          </div>
        </div>
        <button
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
        <div style={{ fontSize: '0.78rem', fontWeight: 600, color: isDark ? '#94a3b8' : '#475569' }}>{t.title}</div>
        <button
          type="button"
          onClick={() => setIsModalOpen(true)}
          style={{
            background: isDark ? 'rgba(255, 255, 255, 0.06)' : 'rgba(255, 255, 255, 0.9)',
            border: isDark ? '1px solid rgba(255, 255, 255, 0.18)' : '1px solid rgba(203, 213, 225, 0.9)',
            borderRadius: '5px',
            padding: '3px 8px',
            fontSize: '0.72rem',
            fontWeight: 600,
