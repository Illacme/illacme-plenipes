...
- **V26.0 [2026-05-13]**: 治理路由单体降解事故复盘。
    - **事故描述**：在拆分 `governance.py` 时，由于未比对 API 响应 Schema 层级且忽略了前端对 `category` ID 的硬编码依赖，导致“插件矩阵”UI 全线崩溃与部分能级显示空白。
    - **固化规制**：新增 SOP-04 第 9 节。强制执行 **Response Schema 100% 对齐** 与 **前端硬编码 ID 全量 Grep 审计**。
    - **技术债清理**：完成了 `core/api/routes/gov/context.py` 的逻辑全路径补完，恢复了 9 大插件分类的动态枚举。
