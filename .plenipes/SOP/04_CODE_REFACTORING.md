...
### IV. GitHub 前置提交锁定与物理预检 (Lock-Before-Cut & Pre-Flight)
- **强制程序**：在启动任何 SOP-04 级重构任务前，AI **必须**主动执行并展示以下命令的结果：
  ```bash
  python3 scripts/pre_flight_check.py
  ```
- **红线拦截**：
    - 若脚本返回 `❌ [拦截]`，AI 严禁对任何 `.py` 或 `.js` 核心文件执行逻辑搬迁。
    - AI 必须向用户报告拦截原因，并立即通过 `git commit & push` 锁定当前基准。
    - 只有当脚本返回 `✅ [准予起飞]` 时，治理准入方可生效。
