...
## 5. 逻辑演进持久化 (Logic Evolution & Blackbox)

> [!CAUTION]
> **代码变动必须具备因果可追溯性。任何缺乏逻辑演进记录的大规模重构均被视为“主权漂移”。**

1. **法定黑匣子**：`.plenipes/history/logic_evolution.log` 是记录逻辑变更意图的法定位置。
2. **记录内容**：必须包含变动的 Action、背后的 RATIONALE (为什么这么改) 以及验证证据 ID。
3. **强制审计**：`sentinel_matrix.py` 会自动核实每次代码变更是否在日志中存在对应记录。
4. **结案陈词**：在每个子任务结束前，AI 必须在该日志中追加一条“结案记录”，标记该逻辑点的“当前稳定态基准 (Stable Baseline)”。
